    ...
    private fun initializeDatabase() {
        try {
            if (databaseHelper == null) {
                databaseHelper = SQLiteAssetHelper(context, winkerkEntry.WINKERK_DB, null, 1)
            }
...
//    """.trimIndent()
    ...
    private fun processCurrentRow(cursor: Cursor, today: DateTime) {
        val firstNameIdx = cursor.getColumnIndex(winkerkEntry.LIDMATE_NOEMNAAM)
        val lastNameIdx = cursor.getColumnIndex(winkerkEntry.LIDMATE_VAN)
        val gemeenteIdx = cursor.getColumnIndex(winkerkEntry.LIDMATE_GEMEENTE)
    ...
    private fun createViewForPosition(position: Int): RemoteViews {
        ...
        if (gemeente.isNotEmpty()) {
            when {
                gemeente == winkerkEntry.GEMEENTE_NAAM -> backgroundColor = winkerkEntry.GEMEENTE_KLEUR
                gemeente == winkerkEntry.GEMEENTE2_NAAM -> backgroundColor = winkerkEntry.GEMEENTE2_KLEUR
                gemeente == winkerkEntry.GEMEENTE3_NAAM -> backgroundColor = winkerkEntry.GEMEENTE3_KLEUR
            }
        }
        ...
        // Set up click intent (opens VerjaarSMS2 when item is tapped)
